<template>
  ...
  >
    ...
    <el-table-column label="操作">
      <template #default="scope">
        <el-link :icon="MoreFilled" @click="on_open_control(scope.row.id)"></el-link>
      </template>
    </el-table-column>
  </el-table>

  <el-drawer v-model="openControl" title="控制面板" :with-header="false" size="50%">
   <el-row>
   ...
   <el-row v-if="currentContainerInfo && currentContainerInfo != ''">
      <dev class="container-code">
        <Codemirror ref="codeMirrorInfoRef" :value="currentContainerInfo" :options="containerInfoOptions" border></Codemirror>
      </dev>
   </el-row>
  </el-drawer>
    <el-dialog title="日志" v-model="logsState.logsDialog" :fullscreen="true" destroy-on-close="true"
        @closed="closeLogsDialog">
      <dev class="container-code">
        <Codemirror ref="codeMirrorRef" :value="logsState.logsTxt" :options="logsState.txtOptions" border></Codemirror>
      </dev>
      <template #footer>
...
import Codemirror, { createLinkMark } from 'codemirror-editor-vue3'
...
const inspect_container = async () => {
    const { code, data, msg } = await (await api.inspectDockerContainerByNode(props.node_name, selectContainerId.value)).data
    if (code !== 0) {
...
const remove_container = async () => {
    const { code, data, msg } = await (await api.removeDockerContainerByNode(props.node_name, selectContainerId.value)).data
    if (code !== 0) {
        ElMessage.error(msg ? msg : '删除容器失败')
        return
    }
...
const logs_container = async () => {
    ...
    try {
        const resp = await api.logsDockerContainerByNode(props.node_name, selectContainerId.value, {}, on_container_logs_chunk, logsState.value.cancel.signal)
        if (resp.status !== 200) {
...
const restart_container = async () => {
    const { code, data, msg } = await (await api.restartDockerContainerByNode(props.node_name, selectContainerId.value)).data
    if (code !== 0) {
        ElMessage.error(msg ? msg : '重启容器失败')
        return
    }
...
const start_container = async () => {
    const { code, data, msg } = await (await api.startDockerContainerByNode(props.node_name, selectContainerId.value)).data
    if (code !== 0) {
        ElMessage.error(msg ? msg : '启动容器失败')
        return
    }
...
const stop_container = async () => {
    const { code, data, msg } = await (await api.stopDockerContainerByNode(props.node_name, selectContainerId.value)).data
    if (code !== 0) {
        ElMessage.error(msg ? msg : '停止容器失败')
        return
    }
...
const get_docker_container_list = async (node_name) => {
    const { code, data, msg } = await (await api.getDockerContainerListByNode(node_name)).data
    if (code !== 0) {
        ElMessage.error(msg ? msg : '获取容器列表失败')
        return
    }
